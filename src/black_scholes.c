...
static double black_scholes_calculation(double S, double K, double T, double r, double sigma, bool is_call, double dividend) {
    ...
    double d_1 = (log(S / K) + T * (r - dividend + (pow(sigma, 2) / 2))) / (sigma * sqrt_T);
    double d_2 = d_1 - (sigma * sqrt_T);

    double option_value;

    if (is_call) {
        option_value = (S * exp(-dividend * T) * phi(d_1)) - (K * exp(-r * T) * phi(d_2));
    } else {
        option_value = (K * exp(-r * T) * phi(-d_2)) - (S * exp(-dividend * T) * phi(-d_1));
    }
